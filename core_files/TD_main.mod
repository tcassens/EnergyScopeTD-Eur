...
set DAYS;			# Number of days 

### parameters
param Nbr_TD default 12; 				#Number of TD days
param Ndata{DAYS,DIMENSIONS}; 			#Input data (already normalized)
var Distance{i in DAYS, j in DAYS}; # := sum{k in DIMENSIONS}((Ndata[i,k]-Ndata[j,k])*(Ndata[i,k]-Ndata[j,k])); 		# Distance matrix.
...
subject to distance_calc {i in DAYS,j in DAYS}:
	Distance [i,j] = sqrt(sum{k in DIMENSIONS}((Ndata[i,k]-Ndata[j,k])*(Ndata[i,k]-Ndata[j,k])));
